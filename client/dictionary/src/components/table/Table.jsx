...
const Table = ({ title, columns, data, groupButton, maxH, limit }) => {
  ...
  return (
    <div className="relative h-full font-roboto text-[#000000E0] w-full">
      ...
      <div
        className={twMerge(
          `relative w-full overflow-auto ${maxH ? `h-[${maxH}px]` : "h-table"}`
        )}
      >
      ...
      <div className="w-full bg-white px-4 py-1" aria-label="Table navigation">
        {data && data.length > 0 && (
          <Pagination totalCount={data.length} limit={limits} />
        )}
      </div>
    </div>
  );
